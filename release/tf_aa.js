...
function aa_aircraft_init()
{
	...
	this.tick=function()
	{
		...
		var dist=this.speed*game.speedFactor*game.frameTime/1000;
		this.y+=dist;
		
		this.recalcBounds();
		var collisions=this.countCollision();
		if (collisions>0) {
			this.game.damagetaken+=collisions*dist;
	...
	this.countCollision=function () {
		...
		for (var i=0;i<this.game.contents.length;i+=1) {
		  ...
		  if (yline-3>ob.ye) continue; //way behind
		  if (zline-1>ob.ze) continue; //above
